...
const addNewExpense = async (idToken, userID, newData) => {
  try {
    const response = await fetch(
      `https://reactreq-default-rtdb.firebaseio.com/users/${userID}/expenses.json?auth=${idToken}`,
      {
        method: "POST",
  ...
  return (
    ...
    <div  className={[styles.card, isDarkMode ? styles.dark : ''].join(' ')} >
        <div  className={styles.welcome}>
          <h1 style={{color:"brown" }}>Welcome To Expense Tracker !!! </h1>
          ...
          >
<div style={{color:"black"}} class="animate__animated animate__heartBeat animate__infinite">Update Your Profile</div>
